...
class AirQualityPredictor:
    ...
    def connect_kafka(self):
        ...
        retry_count = 0
        max_retries = 10  # Increased retries but with shorter delays
        initial_delay = 3  # Start with 3 seconds
        max_delay = 10    # Max 10 seconds between retries
        
        while retry_count < max_retries:
            try:
                logger.info(f"Connecting to Kafka broker at {KAFKA_BROKER} (attempt {retry_count + 1}/{max_retries})")
                ...
                self.producer = KafkaProducer(
                    ...
                    request_timeout_ms=10000  # Timeout for producer operations
                )
                ...
                if retry_count < max_retries:
                    ...
                    delay = min(initial_delay * (2 ** min(retry_count - 1, 2)), max_delay)
...
if __name__ == "__main__":
    # Give Kafka time to start (reduced from 20s to 5s - retry logic handles the rest)
